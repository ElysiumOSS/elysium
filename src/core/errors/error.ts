 ...
 */
export const createTaggedError = <T extends string>(name: T) =>
...
export const ensureBaseError = (
	...
	if (error instanceof Error) {
		return new BaseError(error, command, options?.metadata);
	}

	// Handle non-Error objects
	return new BaseError(new Error(String(error)), command, options?.metadata);
};
